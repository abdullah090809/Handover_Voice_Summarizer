def test_create_care_home_success(client, worker_auth_headers):
...
def test_update_care_home_success(client, worker_auth_headers, test_care_home):
    response = client.put(
...
def test_update_nonexistent_care_home_returns_404(client, worker_auth_headers):
    response = client.put(
        "/care-homes/99999",
...
def test_delete_care_home_success(client, worker_auth_headers, test_care_home):
    care_home_id = test_care_home.id

    response = client.delete(f"/care-homes/{care_home_id}", headers=worker_auth_headers)

    assert response.status_code == 204

    check = client.get(f"/care-homes/{care_home_id}", headers=worker_auth_headers)
    assert check.status_code == 404


def test_delete_nonexistent_care_home_returns_404(client, worker_auth_headers):
    response = client.delete("/care-homes/99999", headers=worker_auth_headers)
